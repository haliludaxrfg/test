...
class Solution {
    public:
        vector<int> getAverages(vector<int>& nums, int k) {
            vector<int> avgs(nums.size(),-1);//初始化为-1
            if(k==0) return nums;//k=0时，直接返回nums
            if(2*k+1>nums.size()) return avgs;//无法满足半径为K
            long long sum = accumulate(nums.begin(),nums.begin()+2*k+1,0LL);
            for(int i =k;i<nums.size()-k;i++){
                avgs[i]=sum/(2*k+1);
                if(i+k+1<nums.size()) sum = sum - nums[i-k]+nums[i+k+1];
            }
            return avgs;
        }
    };